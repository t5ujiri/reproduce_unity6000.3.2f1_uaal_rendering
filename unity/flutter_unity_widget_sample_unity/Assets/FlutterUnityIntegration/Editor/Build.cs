...
namespace FlutterUnityIntegration.Editor
{
    public class Build : EditorWindow
    {
        ...
        private static readonly string WindowsExportPath =
            Path.GetFullPath(Path.Combine(ProjectPath, "../../windows/unityLibrary/data"));

        private static readonly string IOSExportPath =
            Path.GetFullPath(Path.Combine(ProjectPath, "../../ios/UnityLibrary"));

        private static readonly string WebExportPath =
            Path.GetFullPath(Path.Combine(ProjectPath, "../../web/UnityLibrary"));

        private static readonly string IOSExportPluginPath =
            Path.GetFullPath(Path.Combine(ProjectPath, "../../ios_xcode/UnityLibrary"));
        ...
        private static void DoBuildAndroid(String buildPath, bool isPlugin, bool isReleaseBuild)
        {
            ...
            PlayerSettings.SetIl2CppCodeGeneration(NamedBuildTarget.Android,
                isReleaseBuild ? Il2CppCodeGeneration.OptimizeSpeed : Il2CppCodeGeneration.OptimizeSize);
...
";
    ...
    spec.vendored_frameworks = ""UnityFramework.framework""
        ...
        private static bool IsProjectLocationValid(string unityLibraryPath, string platform)
        {
            ...
            {
                Debug.LogError(
                    $"Could not find the Flutter project {platform} folder. Make sure the Unity project folder is located in '<flutter-project>/unity/<unity-project-folder>' .");
